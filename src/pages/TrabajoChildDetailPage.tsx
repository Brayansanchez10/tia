  ...
  return (
    <div className="min-h-[50vh] w-full bg-luxury-bg">
      <div className="mx-auto max-w-[72rem] px-4 pb-16 pt-8 sm:px-5 sm:pb-20 sm:pt-10 md:px-6 md:pt-12">
        <RevealOnView>
          <nav className="mb-8 flex flex-wrap items-center gap-x-2 gap-y-1 text-sm" aria-label="Ruta">
            <Link to="/trabajos" className={crumbLinkClass}>
              Trabajos
            </Link>
            <span className="text-luxury-gold/45" aria-hidden>
              /
            </span>
            <Link to={hubHref} className={crumbLinkClass}>
              {parent.title}
            </Link>
            <span className="text-luxury-gold/45" aria-hidden>
              /
        ...
        <RevealOnView delayMs={80} variant="fadeUp">
          <header className="mx-auto mt-12 max-w-2xl border-t border-white/10 pt-10 text-center md:text-left">
            <p className="m-0 text-[0.7rem] font-semibold uppercase tracking-[0.28em] text-luxury-gold">{child.category}</p>
            ...
            <p className="mt-6 text-xs font-medium uppercase tracking-[0.18em] text-luxury-muted">{child.dateLabel}</p>
            <p className="mt-4 text-sm leading-relaxed text-paper/85 md:text-[0.95rem]">{child.excerpt}</p>
          </header>
        </RevealOnView>
        ...
        <RevealOnView delayMs={120} variant="fadeUp">
          <div className="mx-auto mt-8 max-w-2xl">
            ...
            <div className="mt-10 flex flex-wrap gap-4 border-t border-white/10 pt-8">
              ...
              <ButtonLink variant="luxuryOutline" to="/trabajos" className="rounded-sm px-6 py-2.5 text-xs uppercase tracking-[0.16em]">
                Todos los trabajos
              </ButtonLink>
            </div>
          </div>
